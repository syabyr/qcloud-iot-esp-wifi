...
static void _init_data_template(void)
{
    ...
    sg_DataTemplate[0].data_property.key  = "pm25";
    sg_DataTemplate[0].data_property.data = &sg_ProductData.m_brightness;
    ...
    sg_DataTemplate[1].data_property.key  = "data";
    ...
    sg_ProductData.m_len = 0;
    sg_DataTemplate[2].data_property.key  = "len";
    sg_DataTemplate[2].data_property.data = &sg_ProductData.m_len;
...
typedef struct _PM25DataDefine{
    int nLen;
    uint8_t data[256];
    int nUseFlag;
}PM25DataDefine;
...
static uint8_t *pm25data;
static int nLen;
static int nUseFlag;

static PM25DataDefine sg_PM25Data;

static void _init_pm25_data(void)
{
    //memset((void *) & sg_PM25Data, 0, sizeof(PM25DataDefine));
    pm25data=(uint8_t *) malloc(256);
    memset(pm25data, 0, 256);
    nLen = 0;
    nUseFlag = 0;
}
...
{
    Log_i("---receive down stream user logic");
...
static void cycle_report(Timer *reportTimer)
{
    ...
    if (expired(reportTimer))
    {
        ...
        for (i = 0; i < TOTAL_PROPERTY_COUNT; i++)
        {
            ...
            if (0 == strcmp(pReportData->key, "pm25"))
            {
            ...
            if (0 == strcmp(pReportData->key, "len"))
            {
                int *len = (int *)pReportData->data;
                *len =nLen;

                set_propery_state(pReportData->data, eCHANGED);
            }
            if (0 == strcmp(pReportData->key, "data"))
            {
                char *data=(char *)pReportData->data;
                strncpy(data,(const char *)pm25data, MAX_STR_NAME_LEN);
                //sg_ProductData.m_name[strlen(sg_devInfo.device_name)] = '\0';
...
int qcloud_iot_explorer_demo(eDemoType eType)
{
    ...
    while (IOT_Template_IsConnected(client) || rc == QCLOUD_ERR_MQTT_ATTEMPTING_RECONNECT
        ...
        HAL_SleepMs(1000);
		        ...
		        //获取数据放这里
        nLen=get_data(pm25data,256);
        if((nLen <256) && (nLen >0))
        {
            pm25data[nLen] = 0;
            nUseFlag=1;
        }
        Log_i("data:%d:%s",nLen,pm25data);
    }
